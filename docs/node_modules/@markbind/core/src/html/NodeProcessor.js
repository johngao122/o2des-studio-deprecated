...
class NodeProcessor {
    ...
    processModal(node) {
        if (node.attribs) {
            if (this.processedModals[node.attribs.id]) {
                ...
                (0, vueSlotSyntaxProcessor_1.renameSlot)(node, 'modal-header', 'header');
                (0, vueSlotSyntaxProcessor_1.renameSlot)(node, 'modal-footer', 'footer');
    ...
    processNode(node, context) {
        try {
            ...
            if (_.has(warnings_1.warnDeprecatedAtributesMap, node.name)) {
                warnings_1.warnDeprecatedAtributesMap[node.name](node);
            }
            ...
            switch (node.name) {
                ...
                case 'include':
                    ...
                    return processInclude(node, context, this.pageSources, this.variableProcessor, (text) => this.markdownProcessor.renderMd(text), (text) => this.markdownProcessor.renderMdInline(text), this.config);
                case 'panel':
                ...
                case 'popover':
                    ...
                    return processPopoverSrc(node, context, this.pageSources, this.variableProcessor, (text) => this.markdownProcessor.renderMd(text), this.config);
                case 'tooltip':
    ...
    traverse(node, context) {
        ...
        if (node.name) {
            ...
            if (isHeadingTag && !((_a = node.attribs) === null || _a === void 0 ? void 0 : _a.id)) {
                setHeadingId(node, this.config);
            }
            // Generate dummy spans as anchor points for header[sticky]
            if (isHeadingTag && ((_b = node.attribs) === null || _b === void 0 ? void 0 : _b.id)) {
                (0, cheerio_1.default)(node).prepend(`<span id="${node.attribs.id}" class="anchor"></span>`);
            }
        }
